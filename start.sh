...
case $SERVICE in
  ...
  all)
    ...
    [ -n "$ZOOKEEPER_PID" ] && echo "  [OK] Zookeeper:       localhost:2181 (port client)"
    [ -n "$KAFKA_PID" ] && echo "  [OK] Kafka:           localhost:9092 (broker, pas HTTP)"
    [ -n "$CONSUMER_PID" ] && echo "  [OK] Kafka Consumer:  En cours d'execution"
    echo "============================================================"
    echo ""
    [ -n "$KAFKA_PID" ] && echo "  Note: Kafka n'a pas d'interface web HTTP."
    [ -n "$KAFKA_PID" ] && echo "  Verifier avec: kafka-topics.sh --list --bootstrap-server localhost:9092"
    echo ""
